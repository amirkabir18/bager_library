...
import datetime  # اضافه شده برای پارس صحیح تاریخ میلادی
...
# اصلاح اسکرول‌بار: استفاده از scrollbar.set و اتصال command
...
# اصلاح مشکل خالی بودن جدول در ابتدا:
for row in rows:
...
def search(event=None):
    search_column = combo_column.get()  # تغییر نام برای جلوگیری از تداخل با متغیر گلوبال
    search_value = entry_serch.get()

    if not search_column:
        label_1.config(text="لطفاً یک ستون برای جستجو انتخاب کنید!")
        return
    ...
    try:
        ...
        if not search_value:
            ...
            query = f"SELECT * FROM {tabel_name} WHERE {search_column} LIKE ?"
...
# تکمیل تابع حذف کاربر
def del_member():
    member_to_delete = entry_member_id.get().strip()
    if not member_to_delete:
        messagebox.showwarning("خطا", "لطفاً نام کاربر را برای حذف وارد کنید!")
        return
    
    if messagebox.askyesno("تایید حذف", f"آیا از حذف کاربر {member_to_delete} مطمئن هستید؟"):
        try:
            temp_conn = sqlite3.connect(db_p)
            temp_cursor = temp_conn.cursor()
            temp_cursor.execute("DELETE FROM members WHERE member_id = ?", (member_to_delete,))
            temp_conn.commit()
            temp_conn.close()
...
def on_double_click(event):
    ...
    def refresh_treeview():
        ...
        # استفاده از loans_columns به جای column
        temp_cursor.execute(f"SELECT {', '.join(loans_columns)} FROM {new_tabel_name}")
    ...
    def insert_data():
        ...
        try:
            temp_conn = sqlite3.connect(db_p)
            temp_cursor = temp_conn.cursor()
            temp_cursor.execute('''INSERT INTO loans (member_name, book_id, return_date, borrow_date) VALUES (?, ?, ?, ?)''', 
                                (member_name, book_id if book_id else None, return_date, borrow_date))
            temp_conn.commit()
            temp_conn.close()
    ...
    data_members = [row[0] for row in temp_cursor.fetchall()]
    temp_conn.close()
...
# اتصال دکمه به تابع حذف
member_del = tk.Button(member_frame, text="حذف کاربر", command=del_member, fg="white", font=("B Nazanin", 11, "bold"), width=15, height=1)
...
# تغییر نام به loans_columns برای جلوگیری از تداخل با متغیر column در تابع search
loans_columns = [row[1] for row in new_cursor.fetchall()]

today = jdatetime.date.today()
# اصلاح اسکرول‌بار دوم
loans_tree = ttk.Treeview(tabel_frame, yscrollcommand=scrollbar_2.set, columns=loans_columns, show="headings", height=15)
...
date_col_index = loans_columns.index(borrow_date)
...
for row in loan_rows:
    ...
    if date_col_index != -1 and row_list[date_col_index]:
        try:
            ...
            # اصلاح تبدیل تاریخ: استفاده از datetime استاندارد برای پارس میلادی
...
def history_delete():
    new_cursor.execute("DELETE FROM loans;")
    new_conn.commit()
    refresh_del()
...
h_del = tk.Button(tabel_frame, command=history_delete, text="پاک کردن تاریخچه", width=50)
h_del.pack(pady=3)

# بستن صحیح تمامی اتصالات
new_conn.close()
conn.close()
