...
def insert_member_data():
    ...
    try:
        ...
        temp_cursor.execute(
            "INSERT INTO members (member_id, phone_number) VALUES (?, ?)",
...
btn_register = create_icon_button(
    member_frame,
    text=" ثبت اطلاعات ",
    icon_name="check",
    command=insert_member_data,
    font=FONT_BOLD,
    padx=12,
    pady=4,
)
...
member_tree = ttk.Treeview(
    member_tree_frame,
    yscrollcommand=member_scrollbar.set,
    ...
    def insert_data():
        ...
        try:
            ...
            messagebox.showerror(
                "خطا",
                "فرمت تاریخ بازگشت وارد شده صحیح نیست!\nلطفاً به صورت YYYY-MM-DD وارد کنید.",
            )
            return
        ...
        try:
            ...
            messagebox.showerror(
                "خطا",
                "فرمت تاریخ امانت وارد شده صحیح نیست!\nلطفاً به صورت YYYY-MM-DD وارد کنید.",
            )
            return
        ...
        try:
            ...
            temp_cursor.execute(
                "INSERT INTO loans (member_name, book_id, return_date, borrow_date) VALUES (?, ?, ?, ?)",
                (
                    member_name,
                    book_id if book_id else None,
                    return_date_val,
                    borrow_date_val,
                ),
            )
...
form_cols = [c for c in ["id", "title", "author", "isbn"] if c in columns] + [
...
btn_register_book = create_icon_button(
    book_frame,
    text=" ثبت اطلاعات ",
    icon_name="check",
    command=insert_book_data,
    font=FONT_BOLD,
    padx=12,
    pady=4,
)
...
loans_tree["displaycolumns"] = rtl_display_order(
    loan_column,
    ["id", "member_name", "book_id", "borrow_date", "return_date", "borrowed"],
)
