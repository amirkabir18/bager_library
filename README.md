...
### 📚 ۲. مدیریت پیشرفته مخزن کتاب‌ها و رده‌بندی دیویی (DDC)
- ثبت، ویرایش، حذف و نمایش شناسنامه کامل کتاب‌ها (عنوان، نویسنده، شابک یا ISBN، کد دیویی و موضوع رده).
- **سامانه هوشمند رده‌بندی دهدهی دیویی (Dewey Decimal Classification):**
  - خط لوله چندمرحله‌ای طبقه‌بندی خودکار بر اساس فراداده، دسته‌بندی‌ها، عنوان و واژگان کلیدی.
  - پشتیبانی کامل از رده‌های اصلی ده‌گانه دیویی (۰۰۰ تا ۹۰۰) و زیررده‌های تخصصی.
  - استعلام برخط شابک از Open Library و Google Books و پرکردن خودکار مشخصات کتاب و رده دیویی.
  - نرمال‌سازی هوشمند خط و زبان فارسی/عربی (یکسان‌سازی «ی/ي»، «ک/ك»، نیم‌فاصله، اعراب‌زدایی).
  - مرتب‌سازی سلسله‌مراتبی و عددی دقیق دیویی (جلوگیری از خطاهای مرتب‌سازی متنی/لغوی).
  - پیشنهاد و جانمایی خودکار محل فیزیکی کتاب در قفسه‌های کتابخانه (Shelf Location).
  - اولویت‌بندی رده دستی کاربر (Manual Override) و عدم بازنویسی خودکار آن مگر به درخواست کاربر.
  - قابلیت رده‌بندی دسته‌ای (Batch Reclassification) برای کل مخزن کتاب‌ها با گزارش‌دهی تفکیکی.
- سیستم جستجوی بلادرنگ با فیلترهای منعطف:
  - جستجو در تمام فیلدها یا فیلد اختصاصی (عنوان، نویسنده، کد دیویی، موضوع، شابک).
  - فیلتر تخصصی بر اساس رده‌های موضوعی دیویی (۰۰۰ تا ۹۰۰).
  - فیلتر بر اساس وضعیت در دسترس بودن (امانت داده شده / موجود در کتابخانه).
  - حالت‌های انطباق (شامل بودن، انطباق دقیق، شروع با عبارت).
  - مرتب‌سازی بر اساس کد دیویی (عددی)، عنوان، نویسنده و شناسه به صورت صعودی و نزولی.
